        ...
        # 109082 Couples fusion / 1774 fusion  loi de fusion 2015, 2 années de données 2016/2017
          tabItems(
            ...
            tabItem(tabName = "dashboard1",
                    fluidRow(
                      tabBox(title="",id="test",width = "100%",
                       tabPanel("Matrice de corrélation  ",
                         box(
                           title = "",width = "100%",height = 200,
                           selectInput("choix", "Variable:",c("ellipse","circle","pie"),selected = "ellipse")),
                           plotOutput("plot1", height = 400, width="100%")),
                       tabPanel("dist population  ", plotOutput("POP1", height = 400, width="80%")),
                       tabPanel("dist politique   ", plotOutput("POL", height = 400, width="80%")),
                       tabPanel("dist Navette  ", plotOutput("NAV", height = 400, width="80%"))))
                      ),
  ...
  m<- eventReactive(input$k,{ st_transform(mapCom, "+proj=longlat +datum=WGS84") %>% select(codgeo,P13_POP)%>%filter(.,P13_POP<input$k)})
   ...
   output$ZONE33<-renderLeaflet(
    ...
    observeEvent(input$k,{
      print(m())
      v <- m()
      if(exists("id")){leafletProxy('ZONE33')%>%addPolygons(data=v,color = rainbow(20),layerId = id)}
      else  
      {leafletProxy('ZONE33')%>%removeShape(id)%>%addPolygons(data=v,color = rainbow(20),layerId = id)}
    })
