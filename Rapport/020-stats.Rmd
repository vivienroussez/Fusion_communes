...
On constate qu'il y a beaucoup de couple de communes qui ont une faible diff?rence de population lors de la non-fusion. On retrouve le cas pour les communes rurales ou les grandes villes.  
...
Mais, d'un premier temps, on gardera toutes les variables afin de voir si les m?thodes de ML autre que les regressions sont meilleures. Si oui, on ne travaillera pas les corr?lations.  

Avant de lancer les m?thodes de ML supervis?es, nous allons faire du clustering afin de voir s'il n'existerait pas de clusters de couple de communes.

## Clustering

Etant donné, le nombre important de lignes et la faible capacité de la machine. Nous ne ferons pas de CAH puis un K-means mais un k-means directement. Nous aurions également pu faire un k-means pour diminuer le nombre de lignes et ensuite effectuer une CAH pour maximiser l'inertie interclasse (alors que le k-means minimise l'inertie intraclasse)
...
On voit qu'avec les 6 classes, on retrace pr?s de 90% de la variance totale

On affecte ? chaque couple de communes, son cluster d'appartenance et on regarde dans chaque cluster la r?partition de la variable fusion 
