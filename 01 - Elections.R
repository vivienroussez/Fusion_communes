...
noms <- elect[,c(3,var_nom)] %>% 
      changement_COG_typo(typos=var_nom_lib,codgeo_entree = "codgeo",annees = c(2016,2015))

voix <- elect[,c(3,var_voix)] %>% 
        changement_COG_varNum(var_num = var_voix_lib,codgeo_entree = "codgeo",annees = c(2016,2015))
...
coord <- data.frame(codgeo=rownames(afc$row$coord),afc$row$contrib)
